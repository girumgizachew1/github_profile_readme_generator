...
function LanguagesSection({updateSelectedLanguage}) {
  const [selectedLanguageIcons, setSelectedLanguageIcons] = useState([]);

  const handleLanguageSelect = (language) => {
    setSelectedLanguageIcons((prevIcons) => {
      if (prevIcons.includes(language.icon)) {
        return prevIcons.filter((icon) => icon !== language.icon);
      } else {
        return [...prevIcons, language.icon];
      }
    });
  };
    updateSelectedLanguage(selectedLanguageIcons);
            ...
            <li key={language.name}>
              <label>
                <input
                  ...
                  name="language"
                  value={language.name}
                  checked={selectedLanguageIcons.includes(language.icon)}
